...
// The canonical frame every sequence settles on. Declared once because it is
// three things at once: the manifest's `restFrame`, every sequence's
// `reducedMotion` target, and the frame the mount set below must always carry.
const REST_FRAME = "rest";

// Every frame a consumer has to MOUNT, which is NOT the set the sequence plays.
// A sequence rests on REST_FRAME whether or not its timeline ever names it:
// `running-loop` plays step-a/step-b and rests on `rest`, so a consumer that
// mounts only the played frames renders NOTHING under prefers-reduced-motion.
// Shipped as data rather than left to be derived, because the runtime that most
// needs it is the one with no CSS (React Native), which builds its own player
// from this file and has nothing else to read.
const mountFramesOf = (seq) => [...new Set([...framesOf(seq), REST_FRAME])];
    ...
    Object.entries(SEQUENCES)
      ...
      .map(([k, s]) => [
        k,
        {
          ...
          note: s.note,
          loop: !!s.loop,
          reducedMotion: REST_FRAME,
          // Mount every one of these; play `clip`. The two differ, and taking
          // `clip` for both is what leaves a reduced-motion consumer blank.
          mountFrames: mountFramesOf(s),
          ...(s.idle ? { idle: s.idle } : {}),
          clip: s.clip,
...
function sequenceCss(name, seq) {
  ...
  for (const f of mountFramesOf(seq)) {
...
const animationsCss = `/* Overseer animation sequences. Generated by scripts/gen-sprites.mjs from
   ...
   Stack one frame div for every name in that sequence's \`mountFrames\` (see
   animations.json), give the container a width, and put the sequence class on
   it. \`mountFrames\` is NOT the same as the frames the clip plays: a sequence
   also rests on \`${REST_FRAME}\` when it ends and under reduced motion, even when
   the clip never names it, so mounting only the played poses renders nothing
   once motion is off. The container carries the accessible name; the frames are
   aria-hidden so it reads as one image.
...
// ---------------------------------------------------------------------------
// Motion reference -> exploration/mascot/motion/*.gif + MOTION.md.
//
// The interactive preview above only works in a browser; GitHub renders HTML as
// source. Animated GIFs DO render inline in Markdown, so this is the version
// you can actually look at in the repo, in a PR diff, or on the file browser.
// Same SEQUENCES declaration, so it cannot drift from the shipped timings.
//
// Lives under exploration/, which the determinism rule explicitly exempts from
// byte-stability - a GIF encoder is not something to gate CI on - and which is
// not in package.json `files`, so consumers do not pay for docs in the tarball.
// ---------------------------------------------------------------------------
const MOTION = join(EXPLORE, "motion");
await mkdir(MOTION, { recursive: true });

const GIF_UNIT = 10; // integer multiple of the grid, as always: 180x120
const ONE_SHOT_REST_MS = 1400; // a GIF always loops, so hold on rest to read as a one-shot

const gifFrameCache = new Map();
const gifFrame = async (key) => {
  if (!gifFrameCache.has(key)) {
    gifFrameCache.set(
      key,
      await sharp(Buffer.from(buildSvg(FRAMES[key].map, { unit: GIF_UNIT, label: LABEL })))
        .png()
        .toBuffer(),
    );
  }
  return gifFrameCache.get(key);
};
...
for (const [name, seq] of Object.entries(SEQUENCES)) {
  const steps = timelineOf(seq);
  if (!steps.length) {
    motionRows.push({ name, seq, still: true });
    continue;
  }
  // A one-shot would otherwise machine-gun forever; the trailing rest makes the
  // loop read as "it plays, then settles", which is how consumers fire it.
  const shown = seq.loop ? steps : [...steps, { frame: manifest.restFrame, durationMs: ONE_SHOT_REST_MS }];
  const buffers = [];
  for (const s of shown) buffers.push(await gifFrame(s.frame));
  await sharp(buffers, { join: { across: 1, animated: true } })
    .gif({ delay: shown.map((s) => s.durationMs), loop: 0 })
    .toFile(join(MOTION, `${name}.gif`));
  motionRows.push({ name, seq, still: false });
}
...
const mdTiming = (seq) => {
  ...
  if (seq.idle) {
    const mean = idleMeanMs(seq.idle);
    parts.push(
      `idle \`${seq.idle.minMs}-${seq.idle.maxMs}ms\` ${seq.idle.bias === "square" ? "right-skewed" : "uniform"} (mean ${mean}ms)`,
    );
  }
  if (seq.clip.length) parts.push(seq.clip.map((s) => `\`${s.frame}\` ${s.durationMs}ms`).join(" > "));
  if (seq.repeat) parts.push(`${Math.round(seq.repeat.chance * 100)}% chance of a double`);
  return parts.length ? parts.join(" &middot; ") : "no motion";
};

const motionMd = `# Overseer motion
...
Each clip below is a looping GIF at ${GIF_UNIT}x. One-shot sequences hold on the
...
${motionRows
  .map(({ name, seq, still }) => {
    const img = still
      ? `![${seq.label}](overseer.png)`
      : `![${seq.label}](motion/${name}.gif)`;
    const kind = still ? "static" : seq.loop ? "loops" : "one-shot";
    return `## ${seq.label}\n\n${img}\n\n\`${name}\` &middot; ${kind}\n\n${seq.note}\n\n**Timing:** ${mdTiming(seq)}\n`;
  })
  .join("\n")}
## Rules
...
- Display at **integer multiples** of the ${GRID.columns}x${GRID.rows} grid only.
...
`;
await writeFile(join(EXPLORE, "MOTION.md"), motionMd);
...
console.log(
  `Wrote ${shippedFrames} frames + animations.{json,css} (${shippedSeqs.length} sequences) to assets/mascot/, ` +
    `and ${draftFrames} draft frames, ${hasDrafts ? "candidates.css, " : ""}the demo page, ` +
    `MOTION.md + ${motionRows.filter((r) => !r.still).length} motion GIFs, and ${Object.keys(ALTERNATES).length} alternates to exploration/mascot/`,
);
