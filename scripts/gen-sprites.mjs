...
<style>
  ...
  /* The disembark at 7.92s: the standalone Overseer appears EXACTLY
     ...
     the frame swap is seamless) and sinks one unit per 120ms step. The
     saucer draws above the mascot layer, so the mid rows pass hidden
     behind the hull and the body emerges below it: a descent, not a
     clip-out. The blink idle takes over at 10.8s. */
...
and speeds, never a formation. The Overseer sinks out of the dome one
step at a time, passing behind the hull (the saucer draws above the
mascot layer) and emerging below it to settle on its resting slot; the
empty saucer departs, and the blink idle starts only after settle. On
the site, run the entry on the mascot's side of the page, keep every
waypoint inside the hero's open whitespace, and park the hover clear
below the site header, so the ship never crosses text or controls. On kangentic.com this runs once per
