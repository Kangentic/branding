...
//
// The two-tier rule is applied per rendered size:
//   < 48px  -> F4k board glyph (letterless; survives tiny rasters)
//   >= 48px -> card-K (CUT_SMALL tip below 128, CUT_CANON at 128+)
// Multi-size containers (ICO/ICNS) mix tiers by entry - standard
// detail-tiering, same reason GitHub's favicon drops Octocat detail.
//
// Renditions:
...
import { CREAM, knockout, discOnSquare, partsFor } from "./lib/mark.mjs";
...
const discPng = (size) => {
  const p = partsFor(size);
  return png(knockout(size, p.holes, p.filled));
};
const squarePng = (size, discFrac = 0.82) =>
  png(discOnSquare(size, discFrac, CREAM, partsFor(Math.round(size * discFrac))));
